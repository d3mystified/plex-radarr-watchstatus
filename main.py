...
# Healthchecks.io
HC_BASE = os.getenv('HC_BASE') # e.g. "https://hc-ping.com/12345678-abcd-abcd-abcd-123456789abc"
HC_TIMEOUT = int(os.getenv('HC_TIMEOUT', '10'))

def hc_ping(endpoint: str = '', data: str = '', ok: bool = True) -> None:
    """Ping Healthchecks.io.  ok=False reports failure."""
    if not HC_BASE:
        return
    url = HC_BASE.rstrip('/') + endpoint
    try:
        if data:
            requests.post(url, data=data.encode(), timeout=HC_TIMEOUT)
        else:
...
def main():
    ...
    try:
        ...
        # restore stdout/stderr so the container log still shows everything
        sys.stdout = sys.__stdout__
        sys.stderr = sys.__stderr__
        print(out.getvalue(), end='')
        # ship to Healthchecks
        hc_ping(f'/{exit_code}', out.getvalue(), ok=(exit_code == 0))
        sys.exit(exit_code)
